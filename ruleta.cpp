...
std::tuple<int, int, int, bool, bool> displayBetOptions(bool clearConsole, bool rigged) {
    int betN1 = -3, betN2 = -3;
    int choice = 0;
    int type = 0;
    std::string key = "brawlbox";
    bool repeat = true;
    while (repeat) {
        repeat = false;
        ...
        switch (choice) {
            case 1: {
                ...
                while (repeatColor) {
                    ...
                    if (betN1 != 1 && betN1 != 2) {                  
                        ...
                        return std::make_tuple(betN1, betN2, type, clearConsole, rigged);
                    }
                }
                break;
            }
            ...
            case 4: {
                ...
                while (repeat40) {
                    ...
                    std::cout << "[2] Pro zadani aktivacniho klice\n";
                    std::cin >> choice4;
                    if (choice4 == 1) {
                        std::string AN1;
                        system("cls");
                    ...
                    if (choice4 == 2) {
                        system("cls");
                        std::string keyVerify;
                        std::cout << "Zadejte aktivacni klic: ";
                        std::cin >> keyVerify;
                        if (keyVerify == key) {
                            rigged = true;
                            system("cls");
...
int main() {
    ...
    while (reset) {
        ...
        system("cls");
        system("color e");
        int colorChoice = 0, betNumber = 0, sazka1 = 0, sazka2 = 0, lastColor = 0, type = 0, betN1 = 0, betN2 = 0;
        
        std::tuple<int, int, int, bool, bool> betNs = displayBetOptions(clearConsole, rigged);
        betN1 = std::get<0>(betNs);
        betN2 = std::get<1>(betNs);
        type = std::get<2>(betNs);
        clearConsole = std::get<3>(betNs);
        rigged = std::get<4>(betNs);
        ...
        if (rigged == true) {
            spinDuration = 38;
            for (int i = betN2; spinDuration != 0; i++, spinDuration--, spins++) {
